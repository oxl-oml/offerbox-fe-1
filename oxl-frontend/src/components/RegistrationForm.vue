<template>
  ...
  <form class="border-form-r p-4 border-secondary" @submit.prevent="tryRegister()">
    ...
    <div class="mb-3">
      ...
      <div class="input-group">
        <input v-model="registerData.password2" v-bind:type="[showPassword ? 'text' : 'password']" class="form-control"
               id="inputPassword2"
               placeholder="********" required/>
        <div class="input-group-append">
      ...
      this.register(() => {
        return new HttpHandler().register(this.registerData)
            .then((data: RegisterResponse | DefaultErrorResponse) => {
              const alert: Alert = ((data as RegisterResponse).id) ? (new Alert(AlertTypes.INFORMATION, "Konto zostało utworzone.")) : (new Alert(AlertTypes.ERROR, (data as DefaultErrorResponse).details as string | "Nieznany błąd"))
